...
let info = () => {
	return app.db(db => {
		...
		for (let k in caches) {
			...
			let v = caches[k];
			["transaction", "block", "irreversible_block"].forEach((k) => {
				cacheInfo[k] = cacheInfo[k] + 1;
			});
		}
				...
				try {
					...
					app.db(db => {
						db.models.queues.createSync({
							producer_block_id: k,
							status: caches[k].irreversible_block ? "noreversible" : "reversible",
							isExec: "no",
							data: caches[k]
						});
					});
					...
					console.error("[Error]Queues block event k:%s stack:", k, e.stack);
					throw e;
				} finally {
					delete caches[k];
				}
