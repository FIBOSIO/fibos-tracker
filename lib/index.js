...
function Tracker() {
	...
	let checkBlockNum = (block_num, type) => {
		...
		if (check_num >= block_num) {
			console.warn(util.format("sys block_num(%s) >= node block_num(%s)", check_num, block_num));
			return false;
		}
	...
	this.emitter = (fb) => {
		...
		fibos.on({
			transaction: (trx) => {
				...
				let block = block_caches.get(producer_block_id) || {
					transactions: []
				};
				block.transactions.push({ rawData: trx });
				block_caches.set(producer_block_id, block);
			},
			block: (bk) => {
