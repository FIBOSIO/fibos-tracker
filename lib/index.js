...
function Queues() {
	...
	this.takes = () => {
		...
		confirmed = confirmed.sort((a, b) => {
			return a.block_num - b.block_num > 0 ? 1 : -1;
		});
	...
	this.emitter = (fb) => {
		...
		let emitterEvents = {
			transaction: (trx) => {
				...
				queues.put(producer_block_id, "transaction", trx);
				...
				app.db(db => {
					...
					db.trans(() => {
						...
						queuesList.forEach((queue) => {
							...
							let block_id = db.models.blocks.createSync({
