			...
			block: (bk) => {
				...
				app.db(db => {
					db.trans(() => {
						...
						if (arr.length == 14) {
							if (arr[13].status != "lightconfirm") {
								console.error("arr[13] status error:lightconfirm block_num: %s producer: %s", bk.block_num, bk.block.producer);
								return;
							}
							if (arr[13].producer != arr[12].producer) {
								if (arr[12].status == "lightconfirm") {
									console.error("arr[12] status error:lightconfirm block_num: %s producer: %s", bk.block_num, bk.block.producer);
									return;
								}
							} else {
