...
function Tracker() {
	...
	this.emitter = () => {
		sys_bn = app.db(db => {
			return db.models.tracker_blocks.get_sys_last();
		});
		nore_bn = app.db(db => {
			return db.models.tracker_blocks.get_nore_last();
		})
		if (Config.replay) {
			...
			while (replayStatrBn < sys_bn) {
				app.db(db => {
					...
					let blocks = db.driver.execQuerySync(`select block_num,status,producer_block_id from tracker_blocks where block_num>? order by block_num limit 1000`, [replayStatrBn]);
					db.trans(() => {
						blocks.forEach(bk => {
							let trxs = db.driver.execQuerySync(`select * from tracker_transactions where producer_block_id = ?`, [bk.producer_block_id]);
		...
		fibos.on({
			transaction: (trx) => {
				...
				if (trx.action_traces[0].act.account + "/" + trx.action_traces[0].act.name == "eosio/onblock") return;

				app.db(db => {
					let TrackerTransactions = db.models.tracker_transactions;

					let t = TrackerTransactions.oneSync({
						trx_id: trx.id,
						producer_block_id: trx.producer_block_id,
					})

					if (t) return;
					TrackerTransactions.createSync({
						trx_id: trx.id,
						producer_block_id: trx.producer_block_id,
						rawData: trx
					});
			...
			block: (bk) => {
				...
				app.db(db => {
					let TrackerBlocks = db.models.tracker_blocks;
					...
					while (arr.length < 14 && now_block) {
						...
						now_block = block_caches.get(previous, (previous) => {
							...
							let block = TrackerBlocks.oneSync({
								producer_block_id: previous
							});

							if (!block) {
								block = JSON.parse(fibos.post('/v1/chain/get_block', JSON.stringify({ block_num_or_id: previous })));
								block = {
							...
							let _transactions = db.models.tracker_transactions.find({ producer_block_id: block.producer_block_id }).order("id").runSync();
							return {
					...
					db.trans(() => {
						if (c_block.transactions.length) {
							if (TrackerBlocks.get(bk.producer_block_id)) {
								console.warn("Reentrant block id:", bk.producer_block_id);
								return;
							}

							TrackerBlocks.createSync({
						...
						if (deal_block.length) {
							deal_block.forEach(bk => {
								...
								db.driver.execQuerySync(`update tracker_blocks set status = 'lightconfirm' where producer_block_id = ?`, [bk.producer_block_id]);
								bk.transactions.forEach((trx) => {
									let rawData = trx.rawData;
									let contract_action = rawData.action_traces[0].act.account + "/" + rawData.action_traces[0].act.name;
									db.driver.execQuerySync(`update tracker_transactions set contract_action = ? where producer_block_id = ? and trx_id = ?`, [contract_action, bk.producer_block_id, rawData.id])
									rawData.action_traces.forEach((msg) => { dealData(db, msg); });
								});
							});
						}
					});
			...
			irreversible_block: (blk) => {
				...
				app.db(db => {
					let block = block_caches.get(producer_block_id, (producer_block_id) => {
						let _block = db.models.tracker_blocks.oneSync({
							producer_block_id: producer_block_id
						});
						if (!_block) return null;
						let _transactions = db.models.tracker_transactions.find({ producer_block_id: producer_block_id }).order("id").runSync();
						return {
					...
					db.trans(() => {
						if (block.status === 'reversible') {
							block.transactions.forEach(trx => {
								let rawData = trx.rawData;
								let contract_action = rawData.action_traces[0].act.account + "/" + rawData.action_traces[0].act.name;
								db.driver.execQuerySync(`update tracker_transactions set contract_action = ? where producer_block_id = ? and trx_id = ?`, [contract_action, block.producer_block_id, rawData.id])
								rawData.action_traces.forEach(msg => { dealData(db, msg) });
							})
						}
						...
						db.driver.execQuerySync(`update tracker_blocks set status = 'noreversible' where producer_block_id = ?`, [producer_block_id]);
					})
				})
			}
		});
