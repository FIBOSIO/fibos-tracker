				...
				app.db(db => {
					...
					db.trans(() => {
						...
						queuesList.forEach((queue) => {
							...
							["block_num", "block_time", "producer", "status"].forEach((k) => {
								if (!param[k]) {
									console.error("sys_blocksTable %s is null", k, param);
									fibos.stop();
									process.exit();
								}
							});

							let block_id = sys_blocksTable.createSync(param).id;
							...
							trxs.forEach((trx) => {
								...
								["trx_id", "block_id", "rawData"].forEach((k) => {
									if (!param[k]) {
										console.error("sys_transactionsTable %s is null", k, param);
										fibos.stop();
										process.exit();
									}
								});

								let transaction_id = sys_transactionsTable.createSync(param).id;
								...
								function execActions(at, parent) {
									...
									["contract_name", "action_name", "authorization", "data"].forEach((k) => {
										if (!param[k]) {
											console.error("sys_actionsTable %s is null", k, param);
											fibos.stop();
											process.exit();
										}
									});

									let parent_id = sys_actionsTable.createSync(param).id;
