		...
		timer = setInterval(() => {
			...
			try {
				app.db(db => {
					...
					console.time("runtime-1-trans");

					db.trans(() => {

						let saved = 0;

						console.time("runtime-1-1-queuesList");
						...
						queuesList.forEach((queue) => {
							...
							console.time("runtime-1-1-1-block");
							...
							console.timeEnd("runtime-1-1-1-block");

							console.time("runtime-1-1-2-trxs");
							...
							console.timeEnd("runtime-1-1-2-trxs");

							queues.remove(bk.id);

							saved++;
						});
						console.timeEnd("runtime-1-1-queuesList");

						if (saved) console.notice("len:%s unconfirmed:%s confirmed:%s saved:%s", info.len, info.unconfirmed.length, info.confirmed.length, saved);

						if (irreversibles.length) {
							console.time("runtime-1-2-irreversibles");
							let _irreversibles = util.clone(irreversibles);
							irreversibles = [];
							let affected = sys_blocksTable.updateStatus(_irreversibles);
							console.timeEnd("runtime-1-2-irreversibles");
							console.notice("update irreversibles %s affected %s", _irreversibles.length, affected);
						}
					});
					console.timeEnd("runtime-1-trans");

					console.time("runtime-2-hooks");
					...
					console.timeEnd("runtime-2-hooks");
				});
			} catch (e) {
